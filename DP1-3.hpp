...
template <class T>
class matrix{ /* 行列 : 独自のmat_max_plusというoperator のために定義 */
  vector<vector <T> > mat;
public:
  ...
  friend T max(matrix<T> mat){ /* 最大の要素を返す */
    T m;
    m = max(mat.get(0));
...
public:
  ...
  friend vector<long double> viterbi_compression(char *, char *, int);
};
...
vector<long double> viterbi_compression(char *params, char *data, int block_size){
  hmm model;
  ...
  vector<long double> results(seqs.size(), 0);

  for(int i = 0; i < seqs.size(); i++){
    fourRussian compression(model, seqs.at(i), block_size);
    compression.encoding();
    compression.parsing();
    results.at(i) = compression.propagation();
  }
  return results;
}
